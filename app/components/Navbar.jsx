    ...
    return (
        <>
            <div className='fixed top-0 right-0 w-11/12 -z-10 translate-y-[-80%]'>
                <Image src={assets.header_bg_color} alt='sunjo' className='w-full' />
            </div>
            <nav className='w-full fixed top-0 px-5 lg:px-8 xl:px-[8%] py-4 flex items-center justify-between z-50'>
                <a href='#top'></a>
                <Image src={assets.logo} alt='logo' className="w-28 cursor-pointer mr-24" />
                ...
                <div className='flex items-center gap-4'>
                    ...
                    <a href='#contact' className='hidden lg:flex items-center gap-3 px-10
                     border border-gray-500 rounded-full ml-4 py-2.5 font-Ovo'>Contact <Image src={assets.arrow_icon} alt='icon' className="w-3" /></a>
                    <button className='block md:hidden ml-3' onClick={openMenu}>
                        <Image src={assets.menu_black} alt='menu' className='w-6' />
                    </button>
                </div>
                {/* ------mobile menu -----*/}
                <ul ref={sideMenuRef} className='flex md:hidden flex-col gap-4 py-20 px-10 fixed -right-64 top-0 bottom-0 w-60 
